...
barplot(table(df2$month), col=colors[1])
## dev.off()
```
...
patientsByMonth <- as.vector(table(df2$month))
...
slnbYESsurvivalYES <- length(which(df2$time > 7*365 & df2$SLNB2 == "veica"))
slnbYESsurvivalNO <- length(which(df2$time <= 7*365 & df2$SLNB2 == "veica"))
slnbNOsurvivalYES <- length(which(df2$time > 7*365 & df2$SLNB2 != "veica"))
slnbNOsurvivalNO <- length(which(df2$time <= 7*365 & df2$SLNB2 != "veica"))

df2$survived <- ifelse(df2$time > 7*365, "Survived", "not-Survived")
df2$hasSLNB <- ifelse(df$SLNB2 == "veica", "SLNB", "no-SLNB")
...
chisq.test(df2$hasSLNB, df2$survived, correct=FALSE)
```
...
df2$BreslowStr <- df2$BreslowIndekss
for (i in 1:length(breslow1)) {
  df2$BreslowStr <- str_replace(df2$BreslowStr, 
                               breslow1[i], 
                               breslow2[i])
}
...
df2$BreslowNum <- as.numeric(df2$BreslowStr)

missingPoints <- union(which(is.na(df2$BreslowNum)), which(df2$fixnotikums == "12/31/2022"))
```
...
dfCleared <- df2[setdiff(1:nrow(df2), missingPoints),]
dfMissing <- df2[missingPoints,]
...
missingPoints0 <- which(is.na(df2$BreslowNum))
dfCleared0 <- df2[setdiff(1:nrow(df), missingPoints0),]
dfMissing0 <- df2[missingPoints0,]
